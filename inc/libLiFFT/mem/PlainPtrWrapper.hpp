...
namespace mem {
    ...
    PlainPtrWrapper< UnsignedConst<numDims>,
                     RealOrComplex_t< T_isComplex, types::LibPtr<T, T_isComplex> >,
                     std::false_type,
                     BoolConst<true> >
    wrapLibPtr(T* ptr, const types::Vec<numDims, U>& size)
    {
        constexpr static bool devicePtrTrue = true;
        using LibPtrT = types::LibPtr<T, T_isComplex>;
        return PlainPtrWrapper< UnsignedConst<numDims>,
                                LibPtrT,
                                std::false_type,
                                BoolConst<devicePtrTrue> >(LibPtrT(ptr), size);
    }
